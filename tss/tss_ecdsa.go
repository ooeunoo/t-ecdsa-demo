...
************************
 */
func (ps Parties) InitPartiesWrap(senders []Sender) {
	for i, p := range ps {
		p.Init(ps.NumericIDs(), len(ps)-1, senders[i])
	}
...
func (parties Parties) KeygenWrap() ([][]byte, error) {
	var lock sync.Mutex
...
func (parties Parties) SetShareDataWrap(shareData [][]byte) {
	for i, p := range parties {
		p.SetShareData(shareData[i])
	}
}

func (parties Parties) SignWrap(msg []byte) ([][]byte, error) {
	var lock sync.Mutex
	var sigs [][]byte
	var threadSafeError atomic.Value
